...
bool FileStream::Open(const std::string & path, const StreamMode & mode, const bool& isBinary)
{
	...
	case StreamMode::Write:
		...
		if (out.fail())
		{
			LOG_ERROR("Fail To Open \"%s\" for Writing", path.c_str());
			return false;
		}
		break;
	case StreamMode::Read:
		in.open(path, isBinary? std::ios::in | std::ios::binary : std::ios::in);
		if (in.fail())
		{
			LOG_ERROR("Fail To Open \"%s\" for Reading", path.c_str());
			return false;
		}		break;
	}
